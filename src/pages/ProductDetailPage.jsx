...
function ProductDetailPage() {
  ...
  useEffect(() => {
    dispatch(fetchProductById(productId))
      ...
      .catch(() => {
        toast.error('Product detail could not be fetched.');
      });
  }, [dispatch, productId]);

  if (fetchState === 'FETCHING' && !product) {
    return <LoadingSpinner label="Product detail loading..." />;
  }
          ...
          <div>
            ...
            <div className="mt-6 flex flex-wrap gap-3 text-sm">
              <div className="w-[calc(50%-0.4rem)] rounded-xl bg-slate-100 p-3">
                ...
                <p className="mt-1 text-lg font-bold text-brand-700">{product.price.toFixed(2)} TL</p>
              </div>
              <div className="w-[calc(50%-0.4rem)] rounded-xl bg-slate-100 p-3">
                <p className="text-xs uppercase tracking-wide text-ink-500">Stock</p>
              ...
              <div className="w-[calc(50%-0.4rem)] rounded-xl bg-slate-100 p-3">
                ...
                <p className="mt-1 text-lg font-bold text-ink-900">{product.rating.toFixed(2)}</p>
              </div>
              <div className="w-[calc(50%-0.4rem)] rounded-xl bg-slate-100 p-3">
                <p className="text-xs uppercase tracking-wide text-ink-500">Sell Count</p>
